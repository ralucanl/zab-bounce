...
var FacebookInAppBrowser = {
    ...
    , login: function(successCallback, finalCallback, userIdCallback) {
        ...
        if (this.settings.appId === '' || this.settings.redirectUrl === '') {
            app.showAlert('[FacebookInAppBrowser] You need to set up your app id and redirect url.');
            return false;
        }
        ...
        var faceView,
                callback = function(location) {
            app.showAlert("[FacebookInAppBrowser] Event 'loadstart': " + JSON.stringify(location));
             app.showAlert(location.url)
            if (location.url.indexOf("access_token") !== -1) {
                // Success
                var access_token = location.url.match(/access_token=(.*)$/)[1].split('&expires_in')[0];
                app.showAlert("[FacebookInAppBrowser] Logged in. Token: " + access_token);
            ...
            if (location.url.indexOf("error_reason=user_denied") !== -1) {
                // User denied
                userDenied = true;
                app.showAlert('[FacebookInAppBrowser] User denied Facebook Login.');
                faceView.close();
            }
        },
                userDenied = false;
            ...
            if (location.url == get_url) {
                ...
                face.executeScript({
                    ...
                    if (typeof response[0].id !== 'undefined')
                        window.localStorage.setItem('uid', response[0].id);
                    face.close();
                    if (typeof afterCallback !== 'undefined' && typeof afterCallback === 'function') {
                        setTimeout(function() {
